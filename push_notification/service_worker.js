...
self.addEventListener('push', (event) => {
    ...
    const { title, message, previous_price, current_price, tracker_count, image_path, affiliate_link, history_url, category, urgency, timestamp, product_id } = payload;
    ...
    event.waitUntil(
        Promise.all([
            ...
            trackInteraction('notification_received', product_id)
        ]).catch((error) => {
...
self.addEventListener('notificationclick', (event) => {
    ...
    event.waitUntil(
        handleAction(action, data).then(() => {
            return trackInteraction(interactionType, data.product_id);
        }).catch((error) => {
...
self.addEventListener('notificationclose', (event) => {
    ...
    event.waitUntil(
        trackInteraction('notification_dismissed', data.product_id).catch((error) => {
    ...
    switch (action) {
        ...
        case 'track':
            await fetch('/user/toggle_favorite.php', {
                ...
                body: JSON.stringify({ product_id: data.product_id, is_favorite: true })
            });
            break;
        case 'share':
