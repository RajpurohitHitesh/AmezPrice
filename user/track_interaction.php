...
try {
    ...
    file_put_contents('USER_LOG_PATH', "[" . date('Y-m-d H:i:s') . "] Interaction logged: user_id=$userId, type=$type, product_id=$productId\n", FILE_APPEND);
    echo json_encode(['status' => 'success', 'message' => 'Interaction logged']);
} catch (Exception $e) {
    file_put_contents('USER_LOG_PATH', "[" . date('Y-m-d H:i:s') . "] Error logging interaction: " . $e->getMessage() . "\n", FILE_APPEND);
