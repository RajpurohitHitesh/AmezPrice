...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    // Add to email subscriptions
    $stmt = $pdo->prepare("INSERT INTO email_subscriptions (email, subscribed) VALUES (?, 'yes')");
    $stmt->execute([$email]);

    $stmt = $pdo->prepare("DELETE FROM otps WHERE email = ?");
    $stmt->execute([$email]);

    file_put_contents(__DIR__ . '/../logs/auth.log', "[" . date('Y-m-d H:i:s') . "] User created: $email\n", FILE_APPEND);
