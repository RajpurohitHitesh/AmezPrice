...
if ($action === 'request_otp') {
    ...
    file_put_contents('ADMIN_LOG_PATH', "[" . date('Y-m-d H:i:s') . "] OTP sent to $adminEmail for user deletion\n", FILE_APPEND);
...
if ($action === 'verify_otp') {
    ...
    try {
        ...
        $pdo->commit();
        file_put_contents('ADMIN_LOG_PATH', "[" . date('Y-m-d H:i:s') . "] User $userId deleted by admin $adminEmail\n", FILE_APPEND);
        echo json_encode(['status' => 'success', 'message' => 'User deleted successfully']);
    } catch (Exception $e) {
        $pdo->rollBack();
        file_put_contents('ADMIN_LOG_PATH', "[" . date('Y-m-d H:i:s') . "] Error deleting user $userId: " . $e->getMessage() . "\n", FILE_APPEND);
