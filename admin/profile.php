...
// Debug logging
error_log("Session data on admin profile: " . print_r($_SESSION, true));
...
$stmt = $pdo->prepare("SELECT first_name, last_name, username, email FROM admins WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_profile') {
        ...
        // Check for existing username or email (excluding current admin)
        ...
        if ($email !== $admin['email'] && !$otp) {
            ...
            file_put_contents('ADMIN_LOG_PATH', "[" . date('Y-m-d H:i:s') . "] OTP sent to $email for profile update\n", FILE_APPEND);
        ...
        $stmt = $pdo->prepare("
            ...
            WHERE id = ?
        ");
        $stmt->execute([$firstName, $lastName, $username, $email, $adminId]);

        $stmt = $pdo->prepare("DELETE FROM otps WHERE email = ?");
        $stmt->execute([$email]);

        file_put_contents('ADMIN_LOG_PATH', "[" . date('Y-m-d H:i:s') . "] Profile updated for admin ID $adminId\n", FILE_APPEND);
    ...
    if ($action === 'update_password') {
        ...
        if (!$otp) {
            ...
            $stmt = $pdo->prepare("INSERT INTO otps (email, otp, expires_at) VALUES (?, ?, ?)");
            $stmt->execute([$admin['email'], $otp, $expiresAt]);

            $subject = "Your Password Change Verification Code";
            $message = file_get_contents('../email/templates/otp_email.php');
            $message = str_replace('{{otp}}', $otp, $message);
            sendEmail($admin['email'], $subject, $message, 'otp');

            file_put_contents('ADMIN_LOG_PATH', "[" . date('Y-m-d H:i:s') . "] OTP sent to {$admin['email']} for password change\n", FILE_APPEND);
        ...
        file_put_contents('ADMIN_LOG_PATH', "[" . date('Y-m-d H:i:s') . "] Password updated for admin ID $adminId\n", FILE_APPEND);
...
<body>
    ...
    <script>
        ...
        async function submitProfile(data) {
            const response = await fetch('/admin/profile.php', {
                method: 'POST',
